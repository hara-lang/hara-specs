...
function parseBooleanFilter(value, name) {
  ...
  throw new ApiProblem({
    status: 400,
    code: "INVALID_FILTER",
    message: `${name} must be true, false, all, or omitted.`,
    details: { parameter: name, received: value }
  });
}
...
function parsePageSize(value) {
  ...
  if (!/^\d+$/.test(String(value))) {
    throw new ApiProblem({
      ...
      details: { received: value, maximum: MAX_PAGE_SIZE }
    });
  }
...
export function decodeCursor(value) {
  ...
  try {
    ...
    throw new ApiProblem({
      status: 400,
      code: "INVALID_CURSOR",
      message: "cursor is not a valid Hara specifications cursor."
    });
  }
}
...
export function findSpecification(registry, identifier) {
  let decoded;
  try {
    decoded = decodeURIComponent(String(identifier || "")).trim();
  } catch {
    decoded = String(identifier || "").trim();
  }
...
export function listSpecifications(registry, searchParams) {
  ...
  filtered.sort((left, right) => {
    ...
    return String(left.id || "").localeCompare(String(right.id || ""), "en", { sensitivity: "base", numeric: true }) * direction;
  });
  ...
  if (offset > filtered.length) {
    throw new ApiProblem({
      ...
      message: "cursor points beyond the end of this result set.",
      details: { offset, total: filtered.length }
    });
  }
...
export function paginationLinks(requestUrl, page) {
  ...
  return {
    self: self.href,
    next: withCursor(page.nextCursor),
    previous: withCursor(page.previousCursor)
  };
}
...
export function immutableSource(location) {
  return SHA_PATTERN.test(location?.ref || "");
}

export const registryApiLimits = {
